...
Version control hạ tầng: Lịch sử thay đổi, branch strategy, PR review giống như code.
Reproducibility: Tạo lại hạ tầng dev/prod từ code bất cứ lúc nào.
Team collaboration: Nhiều dev có thể làm việc trên cùng hạ tầng mà không conflict.
Cost tracking & optimization: Dễ thấy resource nào consume chi phí.
Separation of concerns: Network layer riêng (01-network), app layer riêng (02-app).
...
~> 5.0
VPC, subnets, NAT GW, route tables
01-network
terraform-aws-modules/security-group/aws
~> 5.0
SG ALB, ECS, RDS, Redis
02-app
...
Battle-tested: Được cộng đồng dùng lâu, lỗi đã được fix.
Best practices: Input validation, security defaults, tagging.
Reduce maintenance: Không cần maintain custom module code.
Versioning: Explicit version control, pin module version trong code.
Source of truth: Tập trung vào business logic, không reinvent wheel.
...
Bucket policy: Restrict read/write to authorized IAM role chỉ.
VPC endpoint (optional): Terraform apply từ EC2 không qua internet.
Không commit state vào git (add *.tfstate vào .gitignore).
...
Create .terraform/ thư mục (local cache).
Download provider plugin (hashicorp/aws version 5.0).
Download modules từ Terraform Registry (terraform-aws-modules/*).
Create .terraform.lock.hcl (lock file cho version).
Configure backend: Check S3 bucket, DynamoDB lock table, acquire lock.
...
Load code từ .tf files.
Evaluate variables (từ -var-file hoặc environment).
Read state từ S3 (via backend).
Compare desired state (code) vs. actual state (S3).
Generate diff:
+ resource: create
~ attribute: modify
- resource: delete
...
Acquire lock từ DynamoDB (chờ nếu locked).
For each resource in plan: a. Call AWS API (create/update/delete). b. Update state file in memory. c. On success: state persisted to S3. d. On failure: roll back in-memory state, release lock.
Release lock từ DynamoDB.
Output: Outputs value (e.g., vpc_id).
...
CI/CD build image → push ECR.
Terraform update task definition với image URI mới.
Terraform apply → ECS service launch task mới.
Container startup → dotenv.config() load env từ task definition.
Backend connect tới DynamoDB region từ Terraform.
...
Chờ 30 phút auto-release, hoặc terraform force-unlock <LOCK_ID>
data source not found (in 02-app)
01-network chưa apply
Apply 01-network trước
InvalidParameterException: Invalid image
Container image URI sai
Verify ECR image exist, tag correct
AccessDenied to S3/DynamoDB
IAM role chưa có quyền
Add quyền vào IAM policy
terraform plan khác giữa local vs. CI/CD
Terraform version khác
Pin version trong .github/workflows
...
12.2 Best practices
Small commits: Thay đổi logic -> logic commit, infrastructure -> infra commit. Dễ review, dễ rollback.
Plan before apply: Luôn chạy plan, review output trước apply.
Pin module versions: Không dùng >= 5.0, dùng ~> 5.0 (fixed minor).
Tag resources: Mọi resource tagged với Environment, Project, ManagedBy = terraform.
Separate concerns: Network layer riêng, app layer riêng.
Backup state: S3 versioning + cross-region replication.
Lock state: Dùng DynamoDB lock, tránh concurrent apply.
Document variables: Comment rõ input variable, tại sao cần, giá trị default.
...
Check S3 versioning: aws s3api list-object-versions --bucket kicks-shoes-tf-state
Restore từ version cũ: aws s3api get-object --bucket kicks-shoes-tf-state --key dev/02-app/terraform.tfstate --version-id <VERSION_ID> terraform.tfstate
Reupload state: Copy file về local, verify content, push lại S3.
Best practice: Bật S3 versioning + MFA delete để tránh vô tình xóa state.


Q2: Pipeline fail khi apply Terraform, làm sao rollback nhanh?
A:

Quick rollback: Revert PR (sử dụng GitHub revert button), auto-trigger deploy với code cũ.
Manual fix: Edit .tf file, fix lỗi, push lại, auto-deploy.
Force unlock (nếu stuck lock): terraform force-unlock <LOCK_ID> nhưng cẩn thận, chỉ dùng khi chắc lock là stale.
Verify after rollback: terraform plan trước apply để chắc chắn.
...
Build image mới (CI/CD).
Push ECR.
Không cần terraform apply (infrastructure không đổi).
ECS auto-deploy nếu task definition updated (hoặc manual force new deployment).
...
Filter by resource type: terraform plan | grep -E "^(aws_|module\.)" xem resource nào thay đổi.
Check critical resource: Focus on ALB, ECS service, DynamoDB table, security group.
Identify unexpected changes: Nếu plan hiện xóa resource không dự tính → STOP, review trước apply.
Save plan: terraform plan -out=tfplan rồi review file: terraform show tfplan.
